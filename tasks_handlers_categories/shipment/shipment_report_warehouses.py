...
from tasks_handlers_categories.shipment.markups_templates.shipment_report_warehouses_markups import *

from main import bot
from task_handlers import task_info

async def shipment_report_warehouses_date_handler(query: CallbackQuery, user_data, **kwargs):
    task_info["task_subcategory"] = "Отчет по остаткам с разбивкой по складам"
    user_data[query.from_user.id] = {
        "current_message": "shipment_report_warehouses_date",
        "last_bot_message_id": query.message.message_id
    }
    keyboard_markup = await shipment_report_warehouses_date_keyboard("task_delivery")
    user_data["prev_action"] = "report_warehouses_shipment"
...
async def input_shipment_report_warehouses_date_handler(message: types.Message, user_data, **kwargs):
    ...
    keyboard_markup = await shipment_report_warehouses_description_keyboard(user_data["prev_action"])
    ...
    user_data[message.from_user.id] = {
        "current_message": "shipment_report_warehouses_description",
        "last_bot_message_id": sent_message.message_id
    }

#Если дат не ввели
#Ожидание описания задачи
async def input_shipment_report_warehouses_description_handler_without_date(query: CallbackQuery, user_data, **kwargs):
    task_info["task_date"] = "-"
    user_data[query.from_user.id] = {
        "current_message": "shipment_report_warehouses_description",
        "last_bot_message_id": query.message.message_id
...
async def input_shipment_report_warehouses_description_handler(message: types.Message, user_data, **kwargs):
    global task_info
...
#Ожидание подтверждения
async def shipment_report_warehouses_confirmation_handler_without_description(query: CallbackQuery, user_data, **kwargs):
    global task_info
    task_info = clean_task_info(task_info)
    confirmation_message = (
        "Пожалуйста, удостоверьтесь в правильности собранных данных:\n"
