...
from tasks_handlers_categories.content.markups_templates.content_new_card_markups import *
...
async def content_new_card_date_handler(query: CallbackQuery, user_data, **kwargs):
    task_info["photo_paths"] = []
    task_info["document_paths"] = []

    task_info["task_subcategory"] = "Создание карточки"
    user_data[query.from_user.id] = {
        "current_message": "content_new_card_date",
        "last_bot_message_id": query.message.message_id
    }
    keyboard_markup = await content_new_card_info_keyboard("task_content")
    user_data["prev_action"] = "new_card_content"
    
    await query.message.edit_text(text="""Укажите ссылку на документ/таблицу с описание контента для новой карточки или опешите что нужно сделать текстом.\n\nВо избежании возникновения ошибок рекомендуем скопировать [таблицу](https://docs.google.com/spreadsheets/d/1bxNeBY--SLIsKCbQxemW1tOsFPSvKO49uDAbP5gcW0Y/edit#gid=377176396)""",
    reply_markup=keyboard_markup,
    parse_mode='Markdown',
    )
    await query.answer()

async def input_content_new_card_date_handler(message: types.Message, user_data, **kwargs):
    global task_info
    ...
    if is_last_in_album:
        ...
        keyboard_markup = await content_new_card_info_keyboard(user_data["prev_action"])
...
async def input_content_new_card_description_handler_without_date(query: CallbackQuery, user_data, **kwargs):
    ...
    user_data[query.from_user.id] = {
        "current_message": "content_new_card_description",
        "last_bot_message_id": query.message.message_id
    }
    
    keyboard_markup = await content_new_card_description_keyboard(user_data["prev_action"])
...
async def input_content_new_card_description_handler(message: types.Message, user_data, **kwargs):
    global task_info
...
#Ожидание подтверждения
async def content_new_card_confirmation_handler_without_description(query: CallbackQuery, user_data, **kwargs):
    global task_info
