...
from tasks_handlers_categories.analitic.markups_templates.analysis_abc_xyz_markups import *

from main import bot
from task_handlers import task_info

async def analysis_abc_xyz_period_date_handler(query: CallbackQuery, user_data, **kwargs):
    task_info["task_subcategory"] = "Анализ ABC / XYZ"
    user_data[query.from_user.id] = {
        "current_message": "analysis_abc_xyz_date",
        "last_bot_message_id": query.message.message_id
    }
    keyboard_markup = await analysis_abc_xyz_date_keyboard(user_data["prev_action"])
...
async def input_analysis_abc_xyz_period_date_handler(message: types.Message, user_data, **kwargs):
    ...
    keyboard_markup = await analysis_abc_xyz_description_keyboard(user_data["prev_action"])
    ...
    user_data[message.from_user.id] = {
        "current_message": "analysis_abc_xyz_description",
        "last_bot_message_id": sent_message.message_id
    }

#Если дат не ввели
#Ожидание описания задачи
async def input_analysis_abc_xyz_description_handler_without_date(query: CallbackQuery, user_data, **kwargs):
...
async def input_analysis_abc_xyz_description_handler(message: types.Message, user_data, **kwargs):
    ...
    confirmation_message = (
        ...
        f"⚪️ Период анализа: {task_info['period_date']}\n"
...
#Ожидание подтверждения
async def analysis_abc_xyz_confirmation_handler_without_description(query: CallbackQuery, user_data, **kwargs):
