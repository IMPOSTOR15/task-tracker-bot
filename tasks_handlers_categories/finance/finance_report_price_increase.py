...
from tasks_handlers_categories.finance.markups_templates.finance_report_price_increase_markups import *

from main import bot
from task_handlers import task_info

async def finance_report_price_increase_date_handler(query: CallbackQuery, user_data, **kwargs):
    task_info["task_subcategory"] = "Отчет после повышения цен"
    user_data[query.from_user.id] = {
        "current_message": "finance_report_price_increase_date",
        "last_bot_message_id": query.message.message_id
    }
    keyboard_markup = await finance_report_price_increase_date_keyboard(user_data["prev_action"])
    user_data["prev_action"] = "report_increase_financial"
...
async def input_finance_report_price_increase_date_handler(message: types.Message, user_data, **kwargs):
    ...
    keyboard_markup = await finance_report_price_increase_description_keyboard(user_data["prev_action"])
    ...
    user_data[message.from_user.id] = {
        "current_message": "finance_report_price_increase_description",
        "last_bot_message_id": sent_message.message_id
    }

#Если дат не ввели
#Ожидание описания задачи
async def input_finance_report_price_increase_description_handler_without_date(query: CallbackQuery, user_data, **kwargs):
    task_info["task_date"] = "-"
    user_data[query.from_user.id] = {
        "current_message": "finance_report_price_increase_description",
        "last_bot_message_id": query.message.message_id
...
async def input_finance_report_price_increase_description_handler(message: types.Message, user_data, **kwargs):
...
#Ожидание подтверждения
async def finance_report_price_increase_confirmation_handler_without_description(query: CallbackQuery, user_data, **kwargs):
